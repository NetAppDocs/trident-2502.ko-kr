...
예, Trident는 오프라인으로 설치할 수 있습니다. link:trident-install/kubernetes-deploy.html["Trident 설치에 대해 알아봅니다"]을(를) 참조하십시오.
...
필요한 경우 Trident가 설치에 사용하는 정확한 매니페스트 파일을 로컬에서 생성하고 수정할 수 있습니다. link:trident-install/kubernetes-customize-deploy-tridentctl.html["tridentctl 설치를 사용자 지정합니다"]을(를) 참조하십시오.
