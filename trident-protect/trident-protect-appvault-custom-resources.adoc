...
애플리케이션에서 데이터 보호 작업을 수행할 때 명령줄을 사용하거나 수동으로 AppVault CR을 만들어야 하며, AppVault CR은 Trident Protect가 설치된 클러스터에 있어야 합니다.  AppVault CR은 사용자 환경에 따라 다릅니다. AppVault CR을 만들 때 이 페이지의 예를 가이드로 사용할 수 있습니다.
...
CR 또는 Trident Protect CLI 플러그인을 사용하여 AppVault 객체를 생성하는 경우, Restic 및 Kopia 저장소 암호화를 위한 사용자 정의 비밀번호가 포함된 Kubernetes 비밀번호를 사용하도록 Trident Protect에 지시할 수 있습니다. 비밀번호를 지정하지 않으면 Trident Protect는 기본 비밀번호를 사용합니다.
